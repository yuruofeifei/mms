...
class ServingFrontend(object):
			...
			for input in inputs:
				...
				if input_type == 'application/json':
					parameter = {
						'in': 'formData',
						'name': input_name,
						'description': 'Body name: %s with tensor shape: %s' % 
							(input[input_name]['data_name'], input[input_name]['data_shape'][1:]),
						'required': 'true',
						'schema': {
							'type': 'string'
						}
					...
					parameter = {
						'in': 'formData',
						'name': input_name,
						'description': 'Form Data name: %s with image shape: %s' % 
							(input[input_name]['data_name'], input[input_name]['data_shape'][1:]),
						'required': 'true',
						'type': 'file'
					}
	...
	def predict_callback(self, **kwargs):
		...
		input_type = signature['input_type']
		inputs = signature['inputs']
		input_names = [input.keys()[0] for input in inputs]
		output_type = signature['output_type']

		if input_type == 'application/json':
			data = [self.handler.get_form_data(input_name) for input_name in input_names]
		elif input_type == 'image/jpeg':
			data = [self.handler.get_file_data(input_name) for input_name in input_names]
		else:
			raise Exception('%s is not supported for input content-type' % input_type)
